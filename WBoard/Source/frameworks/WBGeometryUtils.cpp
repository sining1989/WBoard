#include "WBGeometryUtils.h"

#include "core/memcheck.h"

const double PI = 4.0 * atan(1.0);
const int WBGeometryUtils::centimeterGraduationHeight = 15;
const int WBGeometryUtils::halfCentimeterGraduationHeight = 10;
const int WBGeometryUtils::millimeterGraduationHeight = 5;
const int WBGeometryUtils::millimetersPerCentimeter = 10;
const int WBGeometryUtils::millimetersPerHalfCentimeter = 5;
const float WBGeometryUtils::inchSize = 25.4f;

WBGeometryUtils::WBGeometryUtils()
{
    // NOOP
}
...
QPolygonF WBGeometryUtils::lineToPolygon(const QLineF& pLine, const qreal& pWidth)
{
    qreal x1 = pLine.x1();
    qreal y1 = pLine.y1();
...
QPolygonF WBGeometryUtils::lineToPolygon(const QLineF& pLine, const qreal& pStartWidth, const qreal& pEndWidth)
{
    qreal x1 = pLine.x1();
    qreal y1 = pLine.y1();
...
QPolygonF WBGeometryUtils::lineToPolygon(const QPointF& pStart, const QPointF& pEnd,
...
QPolygonF WBGeometryUtils::arcToPolygon(const QLineF& startRadius, qreal spanAngleInDegrees, qreal width)
{
 ...
 */
QPolygonF WBGeometryUtils::curveToPolygon(const QList<QPointF>& points, qreal startWidth, qreal endWidth)
{
 ...
 *
 */
QPolygonF WBGeometryUtils::curveToPolygon(const QList<QPair<QPointF, qreal> >& points, bool roundStart, bool roundEnd)
{
    int n_points = points.size();
...
QPointF WBGeometryUtils::pointConstrainedInRect(QPointF point, QRectF rect)
{
    return QPointF(qMax(rect.x(), qMin(rect.x() + rect.width(), point.x())), qMax(rect.y(), qMin(rect.y() + rect.height(), point.y())));
}


QPoint WBGeometryUtils::pointConstrainedInRect(QPoint point, QRect rect)
{
    return QPoint(qMax(rect.x(), qMin(rect.x() + rect.width(), point.x())), qMax(rect.y(), qMin(rect.y() + rect.height(), point.y())));
}


QRectF WBGeometryUtils::lineToInnerRect(const QLineF& pLine, const qreal& pWidth)
{
...
void WBGeometryUtils::crashPointList(QVector<QPointF> &points)
{
 ...
 */
qreal WBGeometryUtils::angle(const QPointF& p1, const QPointF& p2, const QPointF& p3)
{
 ...
 */
QList<QPointF> WBGeometryUtils::quadraticBezier(const QPointF& p0, const QPointF& p1, const QPointF& p2, unsigned int nPoints)
{
